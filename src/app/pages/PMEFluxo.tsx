...
// ── STEP 4: Avaliação de Risco + Detecção de Anomalias ─────────────

type EvalPhase = "analysing" | "done";

const evalMessages = [
  "A calcular score de crédito...",
  "A analisar rácio dívida/receita...",
  "A verificar padrões de fraude...",
  "A cruzar dados com registos externos...",
  "A detectar anomalias documentais...",
  "A gerar relatório de risco...",
];

const riskFactorsPositive = [
  { text: "Situação fiscal regularizada", weight: "+18 pts" },
  { text: "Empresa com mais de 5 anos de actividade", weight: "+15 pts" },
  { text: "Capital social adequado ao montante solicitado", weight: "+12 pts" },
  { text: "Documentos com alta confiança (média 96%)", weight: "+10 pts" },
];

const riskFactorsNegative = [
  { text: "Montante solicitado representa 68% da receita anual", weight: "-8 pts" },
  { text: "Sector de actividade com risco moderado", weight: "-5 pts" },
];

const fraudChecks = [
  { label: "Consistência do NIF",              ok: true,  detail: "NIF válido e activo na base de dados da AGT" },
  { label: "Autenticidade da certidão",         ok: true,  detail: "Documento sem sinais de adulteração detectados" },
  { label: "Correspondência nome/NIF",          ok: true,  detail: "Nome da empresa corresponde ao NIF registado" },
  { label: "Histórico de incidentes",           ok: true,  detail: "Sem registo de incumprimentos anteriores" },
  { label: "Coerência dos valores declarados",  ok: false, detail: "Volume de negócios 22% acima da média sectorial — requer validação adicional" },
  { label: "Padrão de pedido atípico",          ok: true,  detail: "Montante e prazo dentro dos padrões normais para o sector" },
];

function Step4({ onNext }: { onNext: () => void }) {
  const [phase, setPhase] = useState<EvalPhase>("analysing");
  const [progress, setProgress] = useState(0);
  ...
  useState(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (p % 17 === 0 && m < evalMessages.length - 1) {
        m++;
        setMsgIdx(m);
      }
      if (p >= 100) {
  ...
  const score = 74;
  const scoreColor = score >= 70 ? "text-green-600" : score >= 50 ? "text-orange-500" : "text-red-500";
  const scoreLabel = score >= 70 ? "Risco Baixo" : score >= 50 ? "Risco Médio" : "Risco Elevado";
  const scoreBg = score >= 70 ? "bg-green-50 border-green-200" : score >= 50 ? "bg-orange-50 border-orange-200" : "bg-red-50 border-red-200";
  const fraudIssues = fraudChecks.filter((c) => !c.ok).length;
  ...
  if (phase === "analysing") {
    return (
      <div className="bg-white rounded-2xl p-8 border border-border shadow-sm text-center">
        ...
        <h2 className="mb-2">Análise de Risco em curso</h2>
        <p className="text-sm text-coral font-medium mb-6 h-5">{evalMessages[msgIdx]}</p>
        <div className="w-full bg-gray-200 rounded-full h-2.5 mb-2 max-w-sm mx-auto">
          <div className="bg-coral h-2.5 rounded-full transition-all duration-100" style={{ width: `${progress}%` }} />
        </div>
        <p className="text-xs text-muted-foreground">{progress}%</p>
      </div>
    );
  ...
  return (
    <div className="bg-white rounded-2xl p-8 border border-border shadow-sm">
      <div className="inline-flex items-center gap-2 bg-coral/10 text-coral text-[10px] px-3 py-1 rounded-full mb-4">
        PASSO 4 DE 6
      </div>
      <div className="flex items-center gap-2 mb-6">
        <h1>Avaliação de <span className="text-coral">Risco</span></h1>
        <Sparkles className="w-5 h-5 text-coral" />
      </div>

      {/* Score principal */}
      <div className={`border-2 rounded-2xl p-6 mb-6 flex items-center gap-6 ${scoreBg}`}>
        ...
        <div className="flex-1">
          <div className={`text-lg font-bold mb-1 ${scoreColor}`}>{scoreLabel}</div>
          <div className="w-full bg-gray-200 rounded-full h-3 mb-2">
            <div className={`h-3 rounded-full transition-all ${score >= 70 ? "bg-green-500" : score >= 50 ? "bg-orange-400" : "bg-red-500"}`} style={{ width: `${score}%` }} />
          </div>
          <p className="text-xs text-muted-foreground">Score calculado com base em documentos, dados da empresa e padrões históricos</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        <div>
          <h4 className="text-xs font-semibold text-green-600 uppercase tracking-wide mb-3 flex items-center gap-1">
            <CheckCircle className="w-3.5 h-3.5" /> Factores Positivos
          </h4>
          <div className="space-y-2">
            {riskFactorsPositive.map((f, i) => (
              <div key={i} className="flex items-start justify-between gap-2 bg-green-50 border border-green-100 rounded-xl px-3 py-2">
                <p className="text-xs text-foreground leading-relaxed">{f.text}</p>
                <span className="text-xs font-bold text-green-600 flex-shrink-0">{f.weight}</span>
              </div>
            ))}
          </div>
        </div>
        <div>
          <h4 className="text-xs font-semibold text-orange-500 uppercase tracking-wide mb-3 flex items-center gap-1">
            <AlertCircle className="w-3.5 h-3.5" /> Factores de Risco
          </h4>
          <div className="space-y-2">
            {riskFactorsNegative.map((f, i) => (
              <div key={i} className="flex items-start justify-between gap-2 bg-orange-50 border border-orange-100 rounded-xl px-3 py-2">
                <p className="text-xs text-foreground leading-relaxed">{f.text}</p>
                <span className="text-xs font-bold text-orange-500 flex-shrink-0">{f.weight}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="border-2 border-border rounded-2xl p-5 mb-6">
        <div className="flex items-center justify-between mb-4">
          <h4 className="text-sm font-semibold flex items-center gap-2">
            🔍 Detecção de Anomalias e Fraudes
          </h4>
          {fraudIssues === 0
            ? <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full font-medium">Tudo OK</span>
            : <span className="text-xs bg-orange-100 text-orange-700 px-2 py-1 rounded-full font-medium">{fraudIssues} alerta{fraudIssues > 1 ? "s" : ""}</span>
          }
        </div>
        <div className="space-y-2">
          {fraudChecks.map((check, i) => (
            <div key={i} className={`flex items-start gap-3 rounded-xl px-3 py-2.5 border ${check.ok ? "bg-gray-50 border-gray-100" : "bg-orange-50 border-orange-200"}`}>
              {check.ok
      ...
      <div className="bg-green-50 border-2 border-green-200 rounded-2xl p-5 mb-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <p className="text-sm font-bold text-green-700">Candidatura Aprovada com Condições</p>
            <p className="text-xs text-green-600">Score de risco aceitável · 1 alerta de baixa gravidade</p>
          </div>
        </div>
        <div className="grid grid-cols-3 gap-3 mt-4 text-center text-sm">
          <div className="bg-white rounded-xl p-3 border border-green-100">
            <div className="font-extrabold text-green-700">250.000 Kz</div>
            <div className="text-xs text-muted-foreground mt-0.5">Valor Aprovado</div>
          </div>
          <div className="bg-white rounded-xl p-3 border border-green-100">
            <div className="font-extrabold text-green-700">12 meses</div>
            <div className="text-xs text-muted-foreground mt-0.5">Prazo</div>
          </div>
          <div className="bg-white rounded-xl p-3 border border-green-100">
            <div className="font-extrabold text-green-700">~22.000 Kz</div>
            <div className="text-xs text-muted-foreground mt-0.5">Prestação/mês</div>
          </div>
        </div>
      </div>
